...
public class RunToPoseFast extends Action {
    ...
    PositionController xController = new PositionController(0.25, 0);
    PositionController yController = new PositionController(0.25, 0);

    AngleController hController = new AngleController(new BasicPID(new PIDCoefficients(4,0,0)));
    ...
    double startVel = 0, maxVel = 18, maxAccel = 18;

    public RunToPoseFast(Robot robot, Pose2D end) {
        this.robot = robot;
        this.end = end;
    }

    public RunToPoseFast(Robot robot, Pose2D end, double timeout) {
        this.robot = robot;
        this.end = end;
        this.timeout = timeout;
    }

    public RunToPoseFast(Robot robot, Pose2D end, double maxVel, double maxAccel) {
        this.robot = robot;
        this.end = end;
        this.maxVel = maxVel;
    ...
    public RunToPoseFast(Robot robot, Pose2D end, double startVel, double maxVel, double maxAccel) {
        this.robot = robot;
        this.end = end;
    ...
    public RunToPoseFast(Robot robot, Pose2D end, double startVel, double maxVel, double maxAccel, double kP, double kV, double hP) {
        this.robot = robot;
        this.end = end;
    ...
    public void startAction() {
        ...
        if(relativePoint.x >= relativePoint.y) {
            interpolator = new LinearInterpolator(new Point(start.x, 0), new Point(end.x, Angle.normDelta(end.heading - start.heading)));
        }
        else {
            interpolator = new LinearInterpolator(new Point(start.y, start.heading), new Point(end.y, Angle.normDelta(end.heading - start.heading)));
        }
    ...
    public void runAction() throws InterruptedException {
        ...
        robot.driveTrain.driveFieldCentric(xyPower.x, xyPower.y, 0 * -hController.calculate(end.heading, robotPose.heading), Math.toRadians(0));
